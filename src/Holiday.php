...
namespace afiqiqmal\CountryHoliday;
use Goutte\Client;
...
class Holiday
{
	...
	{
      	$this->base_url = "http://www.officeholidays.com/countries/";
	...
	public function getListOfCountries(){
		...
		$result = $crawler->filter('.info-table td a')->each(function ($node){
			$temp['link'] = $node->extract('href')[0]."\n";
			$temp['name'] = str_replace(' ','_',preg_replace('/[^\da-z ]/i','', strtolower(trim($node->text()))));

			return $temp;
		});

		$arrays = array_filter($result);
	...
	private function queryWeb($regional,$year)
	{
		...
		if ($regional == null) 
		{
			$request_url = $this->base_url.$this->country."/".$year.".php";
		}
		else 
		{
			$request_url = $this->base_url.$this->country.$this->regional_path."?list_year=".$currentYear."&list_region=".$regional;
		}
